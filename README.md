...
+ 김지은
  + 모집중 서포팅
    + 모집중 서포팅 목록 출력시 카드 형식
    + 카드의 참여버튼 클릭시 참여인원이 추가되거나 취소되는 기능
    + 서포팅 모집시 air-datepicker api를 사용한 캘린더에서 날짜선택
    + 서포팅 모집시 대표 이미지 미리보기와 다중파일 업로드 기능
    + 그룹이름으로 검색가능
  + 진행중 서포팅
    + 아임포트api를 통해 기타금액을 추가해 결제하는 기능
    + 댓글부분 ajax 구현
    + 그룹이름으로 검색 가능
  + 마이페이지
    + 내가 결제한 서포팅내역을 볼 수 있도록 구현
    + 내가 쓴 서포팅을 볼 수 있도록 구현
